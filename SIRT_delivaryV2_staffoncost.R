...
sirt_admin_work$wau = as.double(sirt_admin_work$wau)
sirt_admin_work %>%
...
sirt_admin_deli$wau = as.double(sirt_admin_deli$wau)
sirt_admin_deli %>%
  summarise(mean = mean(wau),
            sd = sd(wau))
...
############################################################
########### Selecting oly the delivary data
sirt_data = sirt_data %>%
  filter( sirt_workup == 2)
...
sirt_proced = sirt_data %>%
  ...
  filter(redcap_repeat_instrument == "sirt" & sirt_workup == 2) %>%
...
sirt_proced %>% # Recovery time
  filter(sirt_procedure == 2 & !(is.na(timediff_recovery))) %>%
...
sirt_imaging_3 %>%
  ...
  group_by(record_id) %>%
  summarise(num = n()) # Number used imaging in sirt_procedure == 1 --> 0
sirt_imaging_3 %>%
  filter(sirt_procedure == 1) %>% 
  summarise(total_cost = sum(total_cost),
            average = total_cost / 9) #0


sirt_imaging_3 %>%
  filter(sirt_procedure == 2 & num_used > 0) %>%
  group_by(record_id) %>%
  summarise(num = n()) # Number used imaging in sirt_procedure == 1 --> 8
sirt_imaging_3 %>%
  filter(sirt_procedure == 2) %>% 
  summarise(total_cost = sum(total_cost),
            average = total_cost / 8)


sirt_imaging_3 %>%
  filter(sirt_procedure == 3 & num_used > 0) %>%
  group_by(record_id) %>%
  summarise(num = n()) # Number used imaging in sirt_procedure == 1 --> 4
sirt_imaging_3 %>%
  filter(sirt_procedure == 3) %>% 
  summarise(total_cost = sum(total_cost),
            average = total_cost / 4)
...
sirt_pre_ConMed = sirt_data %>%
  filter(sirt_procedure == 1 & sirt_workup == 2) %>%
...
# Aggregate with item
sirt_pre_ConMed_2a = sirt_pre_ConMed_2 %>%
  group_by(item_name) %>%
  summarise(item_cost = median(cost),
            num_items = sum(number_ConMed),
            tot_cot = sum(tot_cost))
write.csv(sirt_pre_ConMed_2a, file = "Consumables/SIRT_W_pre_Con.csv")
...
sirt_intra_ConMed = sirt_data %>%
  filter(sirt_procedure == 2) %>%
  select(1, 3, sirt_intra_deli_5_glucose : sirt_intra_deli_vectorio_l, sirt_intra_deli_fentanyl_1 : sirt_intra_deli_ondansetro)
...
sirt_intra_ConMed_2[is.na(sirt_intra_ConMed_2)]<-0



# Aggregate with item
sirt_intra_ConMed_2a = sirt_intra_ConMed_2 %>%
  group_by(item_name) %>%
  summarise(item_cost = median(cost),
            num_items = sum(number_ConMed),
            tot_cot = sum(tot_cost))
write.csv(sirt_intra_ConMed_2a, file = "Consumables/SIRT_W_intra_Con.csv")
...
sirt_intra_ConMed_4 = sirt_intra_ConMed_4 %>%
  mutate(per_pt = cost_intra_conMed / 8) 
...
# Aggregate with item
sirt_post_ConMed_2a = sirt_post_ConMed_2 %>%
  group_by(item_name) %>%
  summarise(item_cost = median(cost),
            num_items = sum(number_ConMed),
            tot_cot = sum(tot_cost))
write.csv(sirt_post_ConMed_2a, file = "Consumables/SIRT_W_post_Con.csv")
...
sirt_pre_staff = sirt_staff %>%
  filter(redcap_repeat_instance == 4)
sirt_intra_staff = sirt_staff %>%
  filter(redcap_repeat_instance == 5)  
sirt_post_staff = sirt_staff %>%
  filter(redcap_repeat_instance == 6)
...
write.csv(Total_cost, "SIRT_total_Delivary.csv")
...
Total_pre_cost[is.na(Total_pre_cost)]<-0
...
sirt_pre_staff %>%
  summarise(max = max(sirt_staff_al)) # 2
...
sirt_pre_staff %>%
  filter(sirt_staff_al_01_team == "admissions" | sirt_staff_al_01_team == "Admissions") %>%
  summarise(sum = sum(sirt_staff_al_01_hours),
            cost = sum*Administrative_labour)
...
sirt_pre_staff %>%
  filter(sirt_staff_al_02_team == "Infectious Diseases") %>%
  summarise(sum = sum(sirt_staff_al_02_hours),
            cost = sum*Administrative_labour)
...
sirt_pre_staff %>%
  filter(sirt_staff_reg_n_02_team == "Infectious Diseases") %>%
  summarise(sum = sum(sirt_staff_reg_n_02_hours),
            cost = sum*Registered_nurse)
...
sirt_pre_staff %>%
  filter(sirt_staff_reg_n_03_team == "Infectious Diseases") %>%
  summarise(sum = sum(sirt_staff_reg_n_03_hours),
            cost = sum*Registered_nurse)
...
sirt_pre_staff$sirt_staff_fellow_01_team = as.factor(sirt_pre_staff$sirt_staff_fellow_01_team)

levels(sirt_pre_staff$sirt_staff_fellow_01_team)
...
sirt_intra_staff %>%
  filter(sirt_staff_nm_ph_01_team == "Radiology") %>%
  summarise(sum = sum(sirt_staff_nm_ph_01_hours),
            cost = sum*NM_physicist)
...
sirt_post_staff %>%
  filter(sirt_staff_al_01_team == "4C") %>%
  summarise(sum = sum(sirt_staff_al_01_hours),
            cost = sum*Administrative_labour)

sirt_post_staff %>%
  filter(sirt_staff_al_01_team == "4E") %>%
  summarise(sum = sum(sirt_staff_al_01_hours),
            cost = sum*Administrative_labour)
...
sirt_post_staff %>%
  filter(sirt_staff_reg_n_01_team == "4C") %>%
  summarise(sum = sum(sirt_staff_reg_n_01_hours),
            cost = sum*Registered_nurse)

sirt_post_staff %>%
  filter(sirt_staff_reg_n_01_team == "4E") %>%
  summarise(sum = sum(sirt_staff_reg_n_01_hours),
            cost = sum*Registered_nurse)
...
sirt_post_staff %>%
  filter(sirt_staff_reg_n_02_team == "4C ") %>%
  summarise(sum = sum(sirt_staff_reg_n_02_hours),
            cost = sum*Registered_nurse)

sirt_post_staff %>%
  filter(sirt_staff_reg_n_02_team == "4E") %>%
  summarise(sum = sum(sirt_staff_reg_n_02_hours),
            cost = sum*Registered_nurse)
...
sirt_post_staff %>%
  filter(sirt_staff_reg_n_03_team == "4C ") %>%
  summarise(sum = sum(sirt_staff_reg_n_03_hours),
            cost = sum*Registered_nurse)

sirt_post_staff %>%
  filter(sirt_staff_reg_n_03_team == "4E") %>%
  summarise(sum = sum(sirt_staff_reg_n_03_hours),
            cost = sum*Registered_nurse)
...
############## Intern_or_Resident
sirt_post_staff %>%
  summarise(max = max(sirt_staff_in_res)) # 2
...
#2
sirt_post_staff = sirt_post_staff %>%
  mutate(sirt_staff_in_res_02_team = case_when(
    sirt_staff_in_res_02_team %in% 1 ~ "Oncology",
    sirt_staff_in_res_02_team %in% 2 ~ "Radiology",
    sirt_staff_in_res_02_team %in% 3 ~ "Nuclear medicine",
    sirt_staff_in_res_02_team %in% 4 ~ "Anaesthesia"
  ))


sirt_post_staff = sirt_post_staff %>%
  mutate(sirt_staff_in_res_02_team = coalesce(sirt_staff_in_res_02_team, sirt_staff_in_res_02_other))
sirt_post_staff$sirt_staff_in_res_02_team = as.factor(sirt_post_staff$sirt_staff_in_res_02_team)

levels(sirt_post_staff$sirt_staff_in_res_02_team)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "Oncology") %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "GAST" | sirt_staff_in_res_02_team == "Gastro" | sirt_staff_in_res_02_team == "GASTRO" |
           sirt_staff_in_res_02_team == "GAST") %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "HPB") %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "LIVR" ) %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "Radiology" ) %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "RESP" ) %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)

sirt_post_staff %>%
  filter(sirt_staff_in_res_02_team == "Surgery" | sirt_staff_in_res_02_team == "Surgical") %>%
  summarise(sum = sum(sirt_staff_in_res_02_hours),
            cost = sum*Intern_or_Resident)
...
############# Registrar
sirt_post_staff %>%
  summarise(max = max(sirt_staff_registrar)) # 2
...
sirt_post_staff %>%
  filter(sirt_staff_registrar_02_team == "LIVR") %>%
  summarise(sum = sum(sirt_staff_registrar_02_hours),
            cost = sum*Registrar)
